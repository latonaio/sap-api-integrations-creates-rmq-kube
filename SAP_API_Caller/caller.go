...
import (
	...
	sap_api_output_formatter "sap-api-integrations-xxxxxxxx-xxxxxxxx-creates-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	headerData, err := c.callXXXXXXXXSrvAPIRequirementHeader("A_XXXXXXXX", header)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerData, "function": "XXXXXXXXHeader"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callXXXXXXXXSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
	...
	url := strings.Join([]string{c.baseURL, "API_XXXXXXXXX_SRV", api}, "/")
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": outputDataItem, "function": "XXXXXXXXItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
